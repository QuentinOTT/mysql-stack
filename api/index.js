...
// ⚠ Gardées uniquement si elles existent vraiment
//   dans la table players (d’après ton dataset).
// ─────────────────────────────────────────────────
const SORTABLE_COLS = new Set([
  'rk',
  'player','nation','pos','squad','comp','age','born',
  'mp','starts','min',
  'n_90s','gls','ast','g_a','g_pk','pk','pkatt',
  'xg','npxg','xag','npxg_xag',
  'prgc','prgp','prgr',
  'tkl','tklw','blocks','int','tkl_int','clr','err',
  'kp','touches','carries','prgr',
  'crdy','crdr','pkwon','pkcon','recov',
  'ga','saves','save','cs','cs_1','pka','pksv'
]);
...
// Liste brute des players (pour la carte / overview)
app.get('/players', (req, res) => {
  ...
  if (search) {
    conditions.push('(player LIKE ? OR squad LIKE ?)');
    params.push(`%${search}%`, `%${search}%`);
  }
  if (comp) {
    conditions.push('comp = ?');
    params.push(comp);
  }
  if (pos) {
    conditions.push('pos = ?');
  ...
  const safeSort  = SORTABLE_COLS.has(sort) ? sort : 'rk';
...
app.get('/players/table', (req, res) => {
  ...
  db.query(countSql, [...params], (err, countRows) => {
    ...
    db.query(dataSql, [...params, safeLimit, offset], (err2, rows) => {
      ...
      res.json({
        players: rows,
        meta: { total, page: safePage, limit: safeLimit, pages },
      });
    });
  });
});
...
app.get('/cars', (req, res) => {
  ...
  if (search) {
    conditions.push('(company LIKE ? OR model LIKE ?)');
    params.push(`%${search}%`, `%${search}%`);
  }
  if (transmission) {
  ...
  const sql = `SELECT * FROM car_sales ${where} ORDER BY ${safeSort} ${safeOrder} LIMIT ?`;

  db.query(sql, [...params, safeLimit], (err, rows) => {
    if (err) return res.status(500).json({ error: err.message });
    res.json(rows);
  });
});
